...
def plotter2(plot):
    ...
    col = int(col)
    if(col==1):
        colm = input('Enter the column name you want to plot by selecting any column from dataframe head : ')
        if(plot==1):
...
cat = input('Press 1 for plotting all columns or press 2 for specify columns to plot : ')
cat = int(cat)
